...
def _to_device_read(device: Device) -> DeviceRead:
    return DeviceRead(
        id=device.id,
        name=device.name,
...
@router.post("/scan", response_model=list[DeviceRead])
def scan_for_untrusted_devices(
    claims: dict[str, Any] = Depends(get_current_claims),
    db: Session = Depends(get_db),
) -> list[DeviceRead]:
    service = DeviceService(SqlAlchemyDeviceRepository(db))
    devices = service.scan_for_untrusted_devices(_owner_id(claims))
    return [_to_device_read(device) for device in devices]
